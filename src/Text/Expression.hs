...
term more = (parenthesized <|> funcCall <|> nameRef <|>
            (liftM Constant value) <|> choice (map try more)) >>=
        (\e->inSpace (optionMaybe (try (symbol ":" >> unit)) >>= (\u->case u of
            Nothing -> return e
            Just u  -> return $ TypeAssertion e u)))
    where
